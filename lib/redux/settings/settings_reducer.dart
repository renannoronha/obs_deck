...
SettingsState _toggleConnectState(AppState state, ToggleConnectAction action) {
  if (action.toggle) {
    return state.settingsState.copyWith(connectButtonLabel: 'Connected', connectButtonColor: Colors.red);
  } else {
    return state.settingsState.copyWith(connectButtonLabel: 'Connect', connectButtonColor: Colors.grey[300]);
  }
}

SettingsState _toggleStreamState(SettingsState state, ToggleStreamAction action) {
  if (action.toggle) {
    return state.copyWith(streamButtonLabel: 'Streaming', streamButtonColor: Colors.red);
  } else {
    return state.copyWith(streamButtonLabel: 'Start Streaming', streamButtonColor: Colors.grey[300]);
  }
}

SettingsState _toggleRecordState(SettingsState state, ToggleRecordAction action) {
  if (action.toggle) {
    return state.copyWith(recordButtonLabel: 'Recording', recordButtonColor: Colors.red);
  } else {
    return state.copyWith(recordButtonLabel: 'Start Recording', recordButtonColor: Colors.grey[300]);
  }
}

SettingsState _togglePauseState(SettingsState state, TogglePauseAction action) {
  if (action.toggle) {
    return state.copyWith(pauseButtonLabel: 'Resume Recording', pauseButtonColor: Colors.yellow);
  } else {
    return state.copyWith(pauseButtonLabel: 'Pause Recording', pauseButtonColor: Colors.grey[300]);
  }
}

SettingsState _toggleStudioState(SettingsState state, ToggleStudioAction action) {
  if (action.toggle) {
    return state.copyWith(studioButtonColor: Colors.green);
  } else {
    return state.copyWith(studioButtonColor: Colors.grey[300]);
  }
}
